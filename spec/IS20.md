...
`caller` in TxRecord is optional and only need to be non-empty for `transfer_from` calls.
...
```
update transfer_include_fee(to: Principal, value: nat) : TxReceipt
```
...
```
update transfer_from(from: principal, to: principal, value: nat) : TxReceipt
```
...
```
update batch_transfer(transfers: vec record {principal, nat}) : enum { ok : vec nat; err : TxError }
```
...
```
query func get_transaction(index: Nat) : TxRecord
```
...
```
query balance_of(who: principal) : nat
```
...
```
update set_feeTo(newFeeTo: principal)
```
